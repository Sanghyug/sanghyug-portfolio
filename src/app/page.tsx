...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-[#f5f2ec] text-[#1f2a37]">
      ...
      <section
        id="home"
        className="mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-6 pt-28 md:grid-cols-[1.1fr_0.9fr] md:px-12"
      >
        <div>
          ...
          <div className="mt-10 flex gap-3">
            <a
              href="#projects"
              className="rounded-full bg-[#1f2a37] px-6 py-3 text-sm font-bold text-white"
            >
              View Projects
            </a>
            <a
              href="#contact"
              className="rounded-full border border-black/15 px-6 py-3 text-sm font-bold"
            >
              Contact
            </a>
          </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {profileData.aboutMe.map((item) => (
            ...
            >
              ...
              <p className="mt-4 text-sm leading-7 text-[#66707c]">
                {item.desc}
              </p>
            </div>
          ))}
          ...
          {projectsData.map((project) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <p className="text-xs font-bold text-[#a6413a]">
                    PROJECT {project.id}
                  </p>
              ...
              <p className="mt-5 text-sm font-bold text-[#59616d]">
                {project.type}
              </p>
              <p className="mt-3 line-clamp-3 text-sm leading-7 text-[#66707c]">
                {project.desc}
              </p>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {project.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-[#f5f2ec] px-3 py-1 text-xs font-bold text-[#59616d]"
                  >
                    {tag}
                  </span>
                ))}
      ...
      <section id="workflow" className="mx-auto max-w-7xl px-6 py-24 md:px-12">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {profileData.aiWorkflow.map((agent) => (
            <div
              key={agent.num}
              className="rounded-[2rem] border border-black/10 bg-white/75 p-7 shadow-sm"
            >
      ...
      <section className="mx-auto max-w-7xl px-6 pb-4 pt-12 md:px-12">
        {" "}
        <div className="grid gap-8 rounded-[2.5rem] border border-black/10 bg-white/70 p-8 shadow-sm md:grid-cols-[0.35fr_0.65fr] md:p-10">
          {" "}
          <div className="flex items-center gap-5 border-black/10 md:border-r md:pr-8">
            {" "}
            <img
            ...
            <div>
              {" "}
              <p className="text-sm font-bold tracking-[0.25em] text-[#a6413a]">
                {" "}
                BAMNAMU LAB{" "}
              </p>{" "}
              <h3 className="mt-2 text-3xl font-black text-[#1f2a37]">
                {" "}
                밤나무랩{" "}
              </h3>{" "}
              <p className="mt-2 text-sm text-[#66707c]">
                {" "}
                1인 크리에이티브 작업실{" "}
              </p>{" "}
            </div>{" "}
          </div>{" "}
          <div className="text-base leading-8 text-[#59616d]">
            {" "}
            <p>
              {" "}
              어릴 때 가난한 어머니와 밤을 주웠던 적이 있었습니다. 검은
              비닐봉투를 들고 밤나무 밑을 꼼꼼히 뒤졌지만, 다시 그 자리로
              돌아가니 또 밤이 떨어져 있었습니다.{" "}
            </p>{" "}
            <p className="mt-4">
              {" "}
              “방금 주웠는데 또 있어.”라고 말하니 어머니가 “밤나무는 원래
              그래야.”라고 하셨습니다. 그 말이 너무 듣기 좋아서 내내 마음에 품고
              있다가 딸아이 이름을 ‘세상의 밤나무’로 지었습니다.{" "}
            </p>{" "}
            <p className="mt-4 font-bold text-[#1f2a37]">
              {" "}
              세상을 풍요롭게 하는 지혜와 품성을 가지길 바랐기 때문입니다. 1인
              크리에이티브 작업실 이름이 밤나무랩인 이유입니다.{" "}
            </p>{" "}
          </div>{" "}
        </div>{" "}
      </section>
      ...
      <footer id="contact" className="mx-auto max-w-7xl px-6 py-24 md:px-12">
        <div className="rounded-[2.5rem] bg-[#1f2a37] p-10 text-white md:p-14">
          <p className="text-sm font-bold tracking-[0.28em] text-[#d6a19b]">
            CONTACT
          </p>
          ...
          <div className="mt-10 grid gap-4 text-lg md:grid-cols-3">
            <p>
              {profileData.name} / {profileData.englishName}
            </p>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[2rem] bg-white/10 p-6">
            ...
            <p className="mt-3 text-sm leading-7 text-white/70">
              프로젝트 제안, 협업 문의, 투자·지원사업 관련 연락을 남길 수 있는
              공간입니다. 현재는 게시판 UI이며, 실제 저장 기능은 Firebase 또는
              Supabase로 연결할 수 있습니다.
            </p>
            ...
            <div className="mt-6 space-y-3 text-sm">
              <div className="rounded-2xl bg-white/10 p-4">
                ...
                <p className="mt-1 text-white/60">
                  AI-native MVP 제작 및 사업화 협업을 기다립니다.
                </p>
              </div>
              <div className="rounded-2xl bg-white/10 p-4">
...
function SectionTitle({
  ...
  return (
    <div>
      <p className="text-sm font-bold tracking-[0.28em] text-[#a6413a]">
        {eyebrow}
      </p>
